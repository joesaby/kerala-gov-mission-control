...
import type { DraftTone } from "../lib/research-drafts.ts";
import { generateResearchDraft } from "../lib/research-drafts.ts";

interface Props {
  kpis: Kpi[];
  departments: Department[];
  lang: Lang;
  /** USD→INR rate resolved server-side (lib/fx.ts) so drafts match /economy. */
  usdRate: number;
}
...
export default function ResearchExplorer(
  { kpis, departments, lang, usdRate }: Props,
) {
  ...
  const [checkedKpiIds, setCheckedKpiIds] = useState<Record<string, boolean>>(
    kpis[0] ? { [kpis[0].id]: true } : {},
  );
  const [search, setSearch] = useState("");
  const [domainFilter, setDomainFilter] = useState<string>("all");
  const [statusFilter, setStatusFilter] = useState<string>("all");
  const [tone, setTone] = useState<DraftTone>("blog");
  ...
  // Trigger a browser download for a generated blob, then release the URL.
  ...
  const copyToClipboard = (text: string) => {
    navigator.clipboard.writeText(text).then(() => {
      ...
      // textarea remains selectable for a manual copy.
    });
  };
  ...
  const downloadKpiCsv = (kpi: Kpi) => {
    ...
    const rows = kpi.timeSeries
      ...
      .join("\n");
    // BOM so Excel decodes the Malayalam notes as UTF-8.
    const blob = new Blob(["﻿" + headers + rows], {
      type: "text/csv;charset=utf-8;",
    });
    downloadBlob(blob, `${kpi.id}_timeseries.csv`);
  };

  // JSON Exporter for selected KPIs
  const copyKpiJson = (kpi: Kpi) => {
    copyToClipboard(JSON.stringify(kpi, null, 2));
  };
  ...
  const downloadDraftMd = () => {
    ...
    downloadBlob(blob, `KM_Research_Draft_${tone}.md`);
  };
                ...
                <div class="surface-card p-5 space-y-4 sticky top-20">
                  ...
                  <div class="flex gap-2 border-t border-base-200 pt-3">
                    <button
                      ...
                      disabled={!selectedKpi.timeSeries?.length}
          ...
          <div class="surface-card p-5 space-y-4">
            ...
            <label class="flex flex-col gap-1">
              ...
              <select
                value={tone}
                onChange={(e) =>
                  setTone((e.target as HTMLSelectElement).value as DraftTone)}
                class="select select-sm select-bordered w-full"
              >
                <option value="blog">
