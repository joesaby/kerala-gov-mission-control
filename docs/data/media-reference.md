...
- **What it provides:** Structured records for every public figure, political
  party, constituency, government, ministry — with multi-language labels (EN,
  ML, HI, TA), birth dates, party affiliations, position-held intervals (`P39` +
  `P580` start / `P582` end), Commons image filenames.
- **Feeds:** `Minister.nameMl`, `Minister.photoUrl`, party tags, constituency
  labels, and the join key that ties Wikipedia, Commons, and Malayalam Wikipedia
  together.
- **Format:** SPARQL → JSON.
...
- **What it provides:** Long-form prose, cabinet rosters, term tables, reshuffle
  history, biographical summary.
...
- **API:** Same MediaWiki API; pass `&prop=langlinks&lllang=ml` against an EN
  page to get the ML title.
...
- **Feeds:** `Minister.photoUrl`, future `Department.photoUrl` / building photos
  for `/gov/departments/[slug]` heroes.
...
- **What it provides:** One JSON document per page containing extract, original
  image, dates, description. Faster than the full MediaWiki API.
...
- **Sync notes:** Cabinet articles like `Second_Pinarayi_Vijayan_ministry` have
  tabular wikitext that maps cleanly to `Minister` records.
...
- **The Wire / Newslaundry / Newsclick / The News Minute** — long-form political
  reporting and FOI / RTI investigations.
- **The Print's "On My Mind" + ThePrint data** — https://theprint.in
- **PRS Legislative Research** — https://prsindia.org (already in `fiscal.md`).

These media sources are used for **corroboration**, not as primary data. Cite
them in `Kpi.meta.source` only when no government primary source exists for a
number we still want to publish (e.g., a journalist's FOI disclosure).
