...
the dashboard real instead of illustrative. Each file lists endpoints, what
they can populate, update cadence, format, licence, and notes for an automated
sync.

> Rule of the project: if we can't name the source, we can't publish the
> number. Everything we ingest must land in a typed fixture with `source`,
> `sourceUrl`, `updateFrequency`, and `lastRefreshed`.
...
Every source entry follows the same skeleton so a future ingest script can
read this folder as structured metadata if needed:
